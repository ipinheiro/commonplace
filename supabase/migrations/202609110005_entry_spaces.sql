-- Older entries belong to Personal; older saves preserve an existing space.
begin;
...
begin
    ...
    if p_context is not null then
        if jsonb_typeof(p_context) <> 'object'
            ...
            or (p_context - 'tags' - 'url' - 'source' - 'images' - 'date' - 'space') <> '{}'::jsonb
        ...
        if p_context ? 'space' and (jsonb_typeof(p_context->'space') <> 'string'
            or p_context->>'space' not in ('personal', 'work')) then
            raise exception 'Invalid entry space' using errcode = '22023';
        end if;
...
drop function api.list_entries(text, text, timestamptz, uuid, integer, date);
...
begin
    if p_space is null or p_space not in ('personal', 'work')
...
$$;
revoke all on function api.list_entries(text, text, timestamptz, uuid, integer, date, text) from public, anon;
grant execute on function api.list_entries(text, text, timestamptz, uuid, integer, date, text) to authenticated;
